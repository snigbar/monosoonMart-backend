...
const createUserInDB = async function name(user: TUser, path: string) {
  const findUser = await UserModel.isUserExists(user.email);
  if (findUser && findUser.email) {
    fs.unlink(path, (err) => {
      if (err) {
        throw new AppError(
          'user already exists but failed to delete file',
          403,
        );
      }
  ...
  if (result._id && result.email) {
    ...
    const activationLink = `${config.client_URL}/activate/${activationToken}`;
    const html = `
